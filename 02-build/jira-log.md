...
- 2026-07-13 | VS-20 | Ready for Review (D-031 rebuild, supersedes criteria-based) | dev-senior | REBUILT per D-031: Apex managed sharing for BOTH VS_Appointment__c + VS_Patient__c (dropped/deleted the criteria-based sharingRules template). New: VS_Sharing_Group_Id__c (Facility Text18 ExternalId) + User.VS_Facility__c lookup + VS_Facility_Access__c sharing reasons on both objects + VS_FacilityShareService (recompute/provision/membership) + VS_ShareException + VS_FacilityTrigger/Handler (async group provisioning) + VS_AppointmentTrigger/Handler (async share recompute, ZERO SOQL/DML in the §3.4 lock) + VS_AppointmentShareRecalc/VS_FacilityGroupProvisioner/VS_FacilityGroupLinker queueables + VS_FacilityGroupMemberInvocable + 2 test classes (10 methods: staff-A-not-B / patient-at-both / district-View-All+FLS / prune-on-cancel / bulk-200 governor / async-wiring / provisioning+membership). District_Admin+MIS gained read-only FLS (MAJOR-4 fix, required fields omitted); VS_Facility_Staff gained Read+FLS on Patient/Appointment (flagged: SHIP-BAR needs it, VS-04 deferred it here). User membership = Invocable+manual (NOT a global User trigger on the shared DE org, D-031-sanctioned, flagged). metadata-lint = only the 2 pre-existing D-026 $CustomMetadata FAILs (0 new). Builder did NOT deploy/run tests (orchestrator owns the DE-org deploy loop). Org-UNVERIFIED. Build packet: 02-build/review-notes/VS-20-review.md (left the independent VS-20-code-review.md intact).
- 2026-07-13 | VS-20 | Fix applied (deploy pass 1: 0/53 D-028 regression) | dev-senior | Coordinator deploy pass found VS_AppointmentTrigger recompute broke 43 booking/no-show tests (SELECT VS_Facility__c.VS_Sharing_Group_Id__c threw No-such-column under the harness user FLS, D-028) + 10 VS-20 fixture FLS failures. FIX: (a) recomputeSharesById fail-safe try/catch -> booking NEVER breaks on a sharing failure (logs VS_Error_Log__c, applies at runtime on a standard org per D-030a); (b) group resolution now by Group.DeveloperName VS_FAC_<facId> (setup object, no custom-field FLS) instead of VS_Sharing_Group_Id__c; provisionGroups + provisioner also made FLS-safe/fail-safe. Fixtures rewritten to the established permset+runAs D-028 pattern (Booking_Engine+Patient_Service for records, admin-seeded shares via new @TestVisible recomputeSharesInternal) + new TEST-ONLY VS_Facility_Share_Test_Context permset (ManageUsers+FLS on User.VS_Facility__c/VS_Sharing_Group_Id__c). VS_BookingService untouched. metadata-lint 0 new. NOT re-deployed (coordinator owns). Runtime-gated: recompute-success-as-booking-user; all other SHIP-BAR asserts unit-proven.
- 2026-07-14 | VS-20 | Fix applied (deploy pass 2: ManageUsers dependency chain) | dev-senior | ManageUsers on VS_Facility_Share_Test_Context dragged AssignPermissionSets/DelegatedTwoFactor/FreezeUsers etc -> dropped it. Split syncGroupMembership into @TestVisible reconcileGroupMembership(user->facility map, allUserIds); add/prune LOGIC now UNIT-PROVEN directly (no User.VS_Facility__c write); the field-write->membership wiring is RUNTIME-GATED (needs ManageUsers) via syncGroupMembership_publicPath (asserts membership iff the admin field-write succeeds, else the documented no-op). Kept FLS grants + Facility CRUD on the permset. VS_BookingService untouched. metadata-lint 2 pre-existing only. NOT re-deployed (coordinator owns).
- 2026-07-14 | VS-20 | Fix applied (deploy pass 3: VS-20 own-test D-028 + controller interaction) | dev-senior | Booking regression FIXED in pass 2 (all booking/appt/no-show green). Remaining 11: 7 No-such-column VS_Patient__c (recompute/persona SELECT FLS -> runAs fixtureUser w/ Booking+Patient FLS; ship-bar shares hand-seeded as admin so C5.1 negative unit-proven independent of recompute; new TEST-ONLY VS_Facility_Share_Seed_Context object-Modify-All for recompute-based tests; +read FLS on VS_Facility_Staff), 1 tmpVar1 (plain-local binds), 1 writeBack INSUFFICIENT_ACCESS (facC owned by powerUser), 1 getAvailableSlots=0 regression (CONFIRMED benign test-harness interaction - VS-20 never DMLs Slot/Session; bypass triggers in VS_BookingControllerTest @TestSetup). Added lifecycle coverage test. VS_BookingService untouched. metadata-lint 2 pre-existing. NOT re-deployed (coordinator owns).
- 2026-07-14 | VS-20 | Fix applied (deploy pass 4: recompute-success + last tmpVar1) | dev-senior | 51/56 -> fixed last 4. A(1) chosen: VS_Facility_Share_Seed_Context object-Modify-All -> system ModifyAllData (managed-sharing insert requires Modify All Data; object Modify All was insufficient) so the 3 recompute-success tests (lifecycle/async/bulk) now insert __Share for real END-TO-END. tmpVar1 = method-call binds (:stPattern(), :map.keySet()) replaced with plain locals everywhere (test + service resolveFacilityGroups/provisionGroups); scan CLEAN. Surfaced productionization concern: async recompute runs as the enqueuing booking user who lacks Modify All Data in prod -> fail-safes to no-op -> needs an elevated maintainer (scheduled batch/integration user); architect ruling recommended before REQ-053 launch-complete. Ship-bar (hand-seeded admin, C5.1 negative) unchanged + passing. VS_BookingService untouched. metadata-lint 2 pre-existing. NOT re-deployed (coordinator owns). testReads fixed by coordinator (TZ-flaky), not touched.
- 2026-07-14 | VS-16 | Backlog→Ready for Review | dev-senior | Citizen booking-journey: VS_BookingController extended (findMyAppointments/cancelMyAppointment/rescheduleMyAppointment — all OTP-gated + IDOR-safe via assertOwnership BEFORE elevation, D-030 residual MUST-FIX closed) + NOT_APPOINTMENT_OWNER code + VS_Booked_By_Mobile__c ownership stamp (no VS_BookingService change). LWCs: vsSlotPicker refactored (Custom-Label driven, a11y, printable), new vsMyBookings + vsBookingJourney (tabs) + vsErrorLabels; CustomLabels.labels-meta.xml (57 VS_ labels, REQ-060). Permsets: VS_Booking_Portal +Booked_By_Mobile FLS; 2 test-context permsets widened (TEST-ONLY). Tests: VS_BookingControllerManageTest (11) + VS_BookingControllerTest addition — IDOR/OTP-gate/view-filter UNIT-PROVEN; cancel/reschedule counter-write + least-priv wall D-028 runtime-gated. metadata-lint: only the 2 pre-existing D-026 $CustomMetadata FAILs. OQ-030 raised (no named host app). Org-UNVERIFIED (no deploy by dev).
- 2026-07-14 | VS-16 | Ready for Review (deploy fix) | dev-senior | Coordinator deploy: 44/45 green (all security/functional incl. both IDOR rejects; controller 94%). 1 failure resolved TEST-ONLY (VS_BookingService untouched): the least-priv portal cancel proof caught APPOINTMENT_NOT_FOUND not the D-028 wall — ROOT CAUSE: VS_BookingService.cancel's own read enforces sharing on OWD-Private VS_Appointment__c, so a NON-owner portal user can't see the row (org-independent, NOT a D-028 quirk); the without-sharing inner PortalManager elevation does not propagate to the service's read. Split into 2 honest tests: non-owner -> APPOINTMENT_NOT_FOUND (gap asserted); record-owner (booker owns booking, common journey) -> reaches §3.4 counter DML -> D-028 wall. OQ-031 escalated to architect (elevate the service read for mobile-authorized non-owned mgmt? candidate: top-level without-sharing wrapper OR elevated read seam — NOT self-implemented). metadata-lint 2 pre-existing only.
