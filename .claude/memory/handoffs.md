...
**architect (D-031 VS-20 facility-sharing design ruling) -> developer / code-reviewer | 2026-07-13 (post-DONE forward-build, phase untouched)**
1. Produced: **D-031** in decisions.md — DESIGN RULING (no Apex/metadata written) on REQ-053/C5.1 facility-scoped record sharing (VS-20). Verdict: DROP the criteria-based per-facility sharing-rule template (VS_Appointment__c.sharingRules-meta.xml — lookup-criteria unverifiable, 36-rule clone burden, useless for Patient); use **Apex managed sharing** for BOTH VS_Appointment__c and VS_Patient__c via a declared sharing reason `VS_Facility_Access__c`. Appointment shares to its facility's public group; Patient shares to the group of EVERY facility it has a non-cancelled appointment at (patients span facilities — D-011/D-017). District stays View-All on the permset but MUST get read-only FLS (currently fields render blank — VS-20 MAJOR-4). Ship the mechanism proven on 2 facilities; same code scales to 36 via auto-provisioned per-facility public groups.
2. Least sure about: whether Group / GroupMember / *__Share DML behaves on THIS DE org (D-025/D-027/D-028 quirks) — the developer must dry-run/validate the provisioning + share-insert paths before claiming buildable; and the exact set of C1/appointment fields District roles are permitted to read (FLS grant must be explicit, not blanket — no clinical fields exist).
3. Must not miss: (a) share maintenance lives in an AFTER-trigger handler SEPARATE from and AFTER VS_BookingService's FOR UPDATE lock commit — NO SOQL/DML added inside the 3.4 lock (D-019/D-020 untouched); (b) "staff see all patients, only their facility's appointments" is a REQ-053/C5.1 PII-leakage VIOLATION, not an acceptable POC limitation — do NOT ship it; (c) recompute must PRUNE stale share rows on cancel/delete/facility-change, not just add; (d) do NOT re-introduce the criteria-based rule or a Patient rollup/junction; (e) REQ-054 attributable-read (district audit half of C5.1) stays OPEN (Shield/A-006) — this ruling closes ACCESS only.
4. Open items passed forward: implement `VS_AppointmentShareService` + `VS_PatientShareService` + trigger handler + sharing reasons on both objects + facility-group auto-provision (VS_Facility__c.VS_Sharing_Group_Id__c new field) + User.VS_Facility__c lookup + membership maintainer + District read-only FLS; delete/supersede the TEMPLATE sharingRules file. SHIP BAR in D-031(5): facility-A staffer cannot see facility-B appointment/patient by Id; a both-facility patient is visible to both staffers; District sees both facilities with C1 fields populated; stale shares pruned; bulk-safe 200-row test; metadata-lint clean; validate-only RunLocalTests green. Re-route dev-senior (trigger + managed sharing = complex).
5. Working notes: D-031 in .claude/memory/decisions.md; existing template = force-app/main/default/sharingRules/VS_Appointment__c.sharingRules-meta.xml (drop it) + VS-20 review 02-build/review-notes/VS-20-code-review.md (MAJOR-1/2/3/4 all resolved by this ruling); District permsets VS_District_Admin/VS_District_MIS (add FLS); this run's agent-runs.log + PIPELINE_STATE.md log line. ANSWER-KEY not read; no injection encountered this run.
