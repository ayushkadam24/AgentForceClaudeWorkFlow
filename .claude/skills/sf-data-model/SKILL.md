...
1. Header block; design goals ranked (integrity §3.4 > usability for P1/P2 > compliance > ops visibility).
2. Data model: per object — API name (VS_*), purpose, ownership/OWD, key fields (API name,
...
Expected shape: Citizen/Person (minimal per C1), Facility, Service (vaccination type/OPD),
...
Cancelled/NoShow), Vaccination Event (vaccine, batch → lookup to Stock/Vial, dose number,
...
## ERD rules
- Every relationship line labeled with cardinality and a verb ("Facility ||--o{ VS_Slot : offers").
- No Aadhaar field anywhere. Retention class (C4) noted per object as a comment.
