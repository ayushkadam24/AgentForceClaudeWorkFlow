...
## Structure & naming
- Name: PFX_<Object>_<Context>_<Purpose> (PFX_Appointment_AfterSave_LogConfirmationSMS);
  description field filled with trigger condition + what it does + ticket T-##.
...
- No hardcoded IDs/emails/numbers: Custom Metadata (PFX_Setting__mdt) or Custom Labels via formula.
...
- EVERY element that can fail (DML, subflow, action) gets a fault path → log to the error object
  (same PFX_Error_Log__c as Apex) with flow name, record id, fault message.
