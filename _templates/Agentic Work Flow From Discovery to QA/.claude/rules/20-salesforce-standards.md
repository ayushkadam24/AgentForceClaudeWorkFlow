...
## Naming
- Custom objects/fields: prefix `PFX_` (<PROJECT_NAME>), e.g. `PFX_Appointment__c`,
  `PFX_Slot__c.PFX_Capacity__c`. Clear English labels; description filled on EVERY object/field.
- Apex: `PFX_<Domain><Type>` — `PFX_BookingService`, `PFX_SlotSelector`, `PFX_AppointmentTriggerHandler`,
  tests `PFX_BookingServiceTest`. One trigger per object: `PFX_AppointmentTrigger`.
- Flows: `PFX_<Object>_<Trigger>_<Purpose>` e.g. `PFX_Appointment_AfterSave_SendConfirmation`.
  LWC: camelCase `pfxExampleCmp`, `pfxExampleConsole`.
...
- Slot booking MUST use `SELECT ... FOR UPDATE` on the slot row inside the service method that
  checks remaining capacity and inserts the appointment — this is the <CRITICAL_REQ (e.g. RFP §x.y)> guarantee.
- `with sharing` by default; enforce CRUD/FLS (`Security.stripInaccessible` or `WITH USER_MODE`).
- Custom exceptions (`PFX_BookingException`); no empty catch blocks; no hardcoded IDs;
  Custom Metadata (`PFX_Setting__mdt`) or Custom Settings for tunables (cut-off hours, reminder offsets, walk-in reserve %).
...
- Permission sets over profiles for feature access (`PFX_Facility_Staff`, `PFX_Nurse`, `PFX_MO_Facility_Admin`, `PFX_District_Admin`, `PFX_District_MIS`).
