...
## Triggers
- ONE trigger per object, no logic in it: delegate to VS_<Object>TriggerHandler.
...
## Error handling
- Custom exceptions per domain (VS_BookingException, VS_StockException) with message CODES
  ('SLOT_FULL', 'CUTOFF_PASSED') the UI maps to friendly text — never user-facing raw messages.
- try/catch only where you can act on it; log to VS_Error_Log__c (or Platform Event) with class,
...
- @TestSetup data factory (VS_TestFactory) — never org data, no SeeAllData.
